...
/// Cap a rendered match line, keeping context around the first match of
/// `args.query` so callers never receive an unbounded line.
...
/// Cap a single trace/smart region body line so huge single-line files
/// (minified JS, JSON blobs) cannot flood the caller's context window.
...
mod tests {
    ...
    #[test]
    fn compact_match_line_bounds_huge_lines() {
        let line = format!(
    ...
    #[test]
    fn compact_region_body_line_bounds_huge_lines() {
