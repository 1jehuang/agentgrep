...
## Multi-target benchmark matrix

For broader comparisons across a synthetic corpus, `jcode`, and a local Linux source tree:
...
- synthetic corpus: generated at `/tmp/agentgrep-synthetic-bench`
- jcode repo: `/home/jeremy/jcode`
- Linux source tree: `/usr/src/linux`

Environment overrides:

```bash
AGENTGREP_SYNTH_PATH=/tmp/my-corpus \
AGENTGREP_JCODE_PATH=/path/to/jcode \
AGENTGREP_LINUX_PATH=/path/to/linux \
AGENTGREP_BENCH_RUNS=12 \
AGENTGREP_BENCH_WARMUP=3 \
scripts/benchmark_matrix.sh
```
...
```bash
scripts/generate_synthetic_corpus.py /tmp/agentgrep-synthetic-bench
```

It intentionally mixes many small code/docs files with a few highly relevant transcript-related files so we can measure both scale behavior and retrieval usefulness.
